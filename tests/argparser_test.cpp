...
TEST(ArgParserTestSuite, MultiValueTest) {
    ...
    parser.AddIntArgument('p', "param1").MultiValue().StoreValues(int_values); // В СТОР СОХРАНЯЕМ ПРЕОБРАЗОВАННЫЕ ЗНАЧЕНИЯ
...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    ASSERT_TRUE(parser.Parse(SplitString("app --help")));
    // Проверка закоментирована намеренно. Ождидается, что результат вызова функции будет приблизительно такой же,
